...
int main(int argv, const char *argc[]) {
	...
	while(1) {
		...
		arg2 = NULL; // Initialize string argument to NULL
		...
		while (temp != NULL) {
			...
			if (argLen == 1) {
				...
				if (temp[0] == '"') { // Check if the third argument starts with a double quote
					//Check if temp is enclosed in double quotes
					if (temp[strlen(temp)-1] != '"') { // If it does not
						char *temp2 = strtok(NULL, "\"");
						printf("\033[45;34mtemp2: %s\033[0;45m\n", temp2); // Debugging line
						char *temp3 = temp;

						// Increase the allocated size of temp to accommodate the new string
						size_t newSize = strlen(temp) + strlen(temp2) + 3; // 3 for quotes and space
						temp = malloc(newSize);

						// Append temp2 to temp and append a quote at the end
						strcpy(temp, temp3); // Copy the initial part of temp
						strcat(temp, " ");
						strcat(temp, temp2);
						strcat(temp, "\""); // Add the closing double quote
					}

					printf("\033[45;34mtemp: %s\033[0;45m\n", temp); // Debugging line
					arg3 = temp; // Set arg3 to the final value of temp
				}
				else {
					arg3 = temp; // If not enclosed in quotes, treat it as a normal string
				}
			}
			else if (argLen == 4) {
				...
				printf("\033[45;31mError: Too many arguments for command '\033[45;33m%s\033[45;31m'.\033[0;45m\n", command);
				break; // Exit the loop if more than 4 arguments are provided
			}
